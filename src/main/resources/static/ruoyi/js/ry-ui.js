...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            primaryField: function (value, searchValue) {
                var actions = [];
                if (value == searchValue || value.indexOf(searchValue) >= 0) {
            ...
            getProgress: function (value) {
                var actions = [];
                actions.push("<small>当前进度 : " + value + "%</small>")
                actions.push("<div class='progress progress-mini'>");
                actions.push("<div class='progress-bar' style='width:" + value + "%'>");
                actions.push("</div>");
                actions.push("</div>");
            ...
            //根据用户ID 显示头像
            getImg: function (value, userPrefix) {
                var url = userPrefix + '/userListByIds';
                var actions = [];
                var data = {'ids': value};
                $.ajax({
                    type: "post",
                    ...
                    success: function (rep) {
                        actions.push("<div class='project-people'  style='text-align:left'>");

                        $.each(rep.data, function (i, val) {
                            actions.push("<a title='"+val.userName+"' href='"+val.avatar+"'  target='_blank' style='margin-left:6px' >");
                            actions.push("<img  class='img-circle' src='"+val.avatar+"' />");
                            actions.push("</a>");
                        });
